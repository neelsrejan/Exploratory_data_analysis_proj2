...
for (year_in in years) {
    average[count] <- mean(NEI[NEI$year == year_in & NEI$fips == 24510 & NEI$type == "ON-ROAD", "Emissions"], na.rm = TRUE)
    count <- count + 1
}
...
g <- ggplot()
g <- g + xlab("Year") + ylab("Baltimore PM2.5 motor vehicle emission") + 
    ggtitle("Baltimore PM2.5 motor vehicle emission per year") +
    geom_line(data = average_pm2.5_by_year_baltimore, aes(x = Year, y = Average_emission, color = "red"), show.legend =  FALSE)
